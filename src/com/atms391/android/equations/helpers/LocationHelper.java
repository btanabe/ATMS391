...
public class LocationHelper {
	...
	public static String getPrettyPrintedLatitude(double latitude){
		...
		if(latitude > 0){									// In the northern hemisphere
			stringBuilder.append(NumberPrinterHelper.roundToTwoDecimalPlaces(latitude));
			stringBuilder.append("°");
			stringBuilder.append("N");
		} else if(latitude < 0){							// In the southern hemisphere
			stringBuilder.append(NumberPrinterHelper.roundToTwoDecimalPlaces(Math.abs(latitude)));
			stringBuilder.append("°");
	...
	public static String getPrettyPrintedLongitude(double longitude){
		...
		if(longitude > 0){									// In the eastern hemisphere
			stringBuilder.append(NumberPrinterHelper.roundToTwoDecimalPlaces(longitude));
			stringBuilder.append("°");
			stringBuilder.append("E");
		} else if(longitude < 0){							// In the western hemisphere
			stringBuilder.append(NumberPrinterHelper.roundToTwoDecimalPlaces(Math.abs(longitude)));
			stringBuilder.append("°");
